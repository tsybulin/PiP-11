...
static bool isspace(unsigned char s) {
    return s == ' ' || s == '\t' || s == '\n' || s == '\r' || s == '\v' || s == '\f' ;
}
...
FRESULT ini_parse_stream(ini_reader reader, void* stream, ini_handler handler, void* user) {
    ...
    while (reader(line, (int)max_line, stream) != 0) {
        lineno++;
...
FRESULT ini_parse_file(FIL* file, ini_handler handler, void* user) {
...
FRESULT ini_parse(const char* filename, ini_handler handler, void* user) {
    ...
    if (result != FR_OK) {
		gprintf("f_open(%s) error:  (%d)", filename, result);
        return result ;
    }
