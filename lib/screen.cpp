...
void CScreenDevice::scrollRegion(u8 top_limit, u8 bottom_limit, int offset, TScreenColor bg_color) {
	...
	u8 rows = bottom_limit - top_limit + 1 ;
	u32 *pFrom, *pTo ;
	unsigned nSize  ;

	if (offset > 0) {
		// scroll up
		int pfrom = (top_limit + offset) * m_CharGen.GetCharHeight() ;
		int pto = top_limit * m_CharGen.GetCharHeight() ;
		pFrom = (u32 *) (m_pBuffer + pfrom * m_nPitch);
		pTo = (u32 *) (m_pBuffer + pto * m_nPitch);
		nSize = m_nPitch * (rows - offset) * m_CharGen.GetCharHeight() * sizeof (TScreenColor) ;
		m_DMAChannel.SetupMemCopy(pTo, pFrom, nSize, SCREEN_DMA_BURST_LENGTH, FALSE);
		m_DMAChannel.Start() ;
		m_DMAChannel.Wait() ;
	} else {
		// scroll down
		int pfrom = top_limit * m_CharGen.GetCharHeight() ;
		int pto = (top_limit - offset) * m_CharGen.GetCharHeight() ;
		pFrom = (u32 *) (m_pBuffer + pfrom * m_nPitch);
		pTo = (u32 *) (m_pBuffer + pto * m_nPitch);
		nSize = m_nPitch * (rows + offset) * m_CharGen.GetCharHeight() * sizeof (TScreenColor) ;
		memmove(pTo, pFrom, nSize) ;
	}
	
	if (offset > 0) {
		int pfrom = (bottom_limit - offset + 1) * m_CharGen.GetCharHeight() ;
		pFrom = (u32 *) (m_pBuffer + pfrom * m_nPitch);
		nSize = m_nPitch * offset * m_CharGen.GetCharHeight() * sizeof (TScreenColor) ;
		memset(pFrom, bg_color, nSize) ;
	} else {
		int pfrom = top_limit * m_CharGen.GetCharHeight() ;
		pFrom = (u32 *) (m_pBuffer + pfrom * m_nPitch);
		nSize = m_nPitch * -offset * m_CharGen.GetCharHeight() * sizeof (TScreenColor) ;
...
void CScreenDevice::displayChar(char chChar, unsigned col, unsigned row, TScreenColor fg, TScreenColor bg)  {
	unsigned px = col * m_CharGen.GetCharWidth() ;
	unsigned py = row * m_CharGen.GetCharHeight() ;
	for (unsigned y = 0; y < m_CharGen.GetCharHeight(); y++) {
		for (unsigned x = 0; x < m_CharGen.GetCharWidth(); x++) {
			SetPixel(px + x, py + y,  m_CharGen.GetPixel(chChar, x, y) ? fg : bg);
		}
	}
}
...
unsigned CScreenDevice::getCharWidth() {
	return m_CharGen.GetCharWidth() ;
}

unsigned CScreenDevice::getCharHeight() {
	return m_CharGen.GetCharHeight() ;
}

void CScreenDevice::ClearScreen() {
	CursorHome ();
	ClearDisplayEnd ();
	InvertCursor() ;
}
